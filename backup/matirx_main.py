...
def parse_args():
    ...
    parser.add_argument('--data_path', type=str, default='D:\\data\\online')
...
def main():
    ...
    for class_name in mvtec.CLASS_NAMES:
        ...
        if not os.path.exists(train_feature_filepath) or args.research:
            ...
            kcluster.fit_network(embedding_vectors)
            # ng = NeuralGas.OnlineNeuralGas(embedding_vectors)
            # ng.fit_network(embedding_vectors)
            # ng.cluster_label()
            # ng.del_null_data_nodes()
            # mean = torch.mean(embedding_vectors, dim=0).numpy()
            # cov = torch.zeros(C, C, H * W).numpy()
            # I = np.identity(C)
            # for i in tqdm(range(H * W)):
            #     # cov[:, :, i] = ShrunkCovariance().fit(embedding_vectors[:, :, i].numpy()).covariance_
            #
        ...
        for (x, y, mask) in tqdm(test_dataloader, '| feature extraction | test | %s |' % class_name, position=1):
            ...
            for i in tqdm(range(B * H * W), '| anomaly detection | test | %s |' % class_name, position=0):
                ...
                neighbor_dist = []
                ranking = ng.gpu_find_nearest_units(embedding_vectors[i, :])
                # 取获胜节点邻域内最小的马氏距离
                s1 = ranking[0][0]
                mean = ng.network.nodes[s1]['paras']['mean']
                if type(mean) is np.ndarray:
                    conv_inv = np.linalg.inv(ng.network.nodes[s1]["paras"]["cov"])
                    dist = Mahalanobis(embedding_vectors[i, :].reshape(1, -1), mean, conv_inv)
                    neighbor_dist.append(dist)
                for u in ng.network.neighbors(s1):
                    mean = ng.network.nodes[u]['paras']['mean']
                    if type(mean) is np.ndarray:
                        conv_inv = np.linalg.inv(ng.network.nodes[u]["paras"]["cov"])
                        dist = Mahalanobis(embedding_vectors[i, :].reshape(1, -1), mean, conv_inv)
                        neighbor_dist.append(dist)
                    else:
                        continue
                dist_list.append(np.min(neighbor_dist))
            ...
            ng.fit_network(embedding_vectors, online=True)
            ng.del_nodes()
            ng.cluster_label()
            ng.del_null_data_nodes()
            #
        ...
        print('Average PRO: %.3f' % np.mean(total_pro_auc))
    #
    # fig.tight_layout()
...
def Mahalanobis2(x, mean, cov):
    B, C = x.shape
    x = x.reshape(B, 1, C)
    mean = mean.reshape(1, -1, C)
    delta = (x - mean).reshape(B, -1, 1, C)
    cov = np.expand_dims(cov, 0).repeat(B, axis=0)
    temp = np.einsum("izjk,izkn->izjn", delta, cov)
    return np.sqrt(np.einsum("izjk,izkn->izjn", temp, delta.transpose(0, 1, 3, 2))).squeeze()
...
def Mahalanobis(x, mean, cov):
    ...
    cov = cov.reshape(1, -1, C)
    delta = (x - mean).reshape(B, 1, C)
    temp = np.einsum("ijk,ikn->ijn", delta, cov)
    # pdb.set_trace()
    return np.sqrt(np.einsum("ijk,ikn->ijn", temp, delta.transpose(0, 2, 1))).squeeze()
