...
class TrainingHistory(object):
    ...
    def plot(self, colors=None, linestyles=None, linewidths=None, markers=None, y_lim=(0, 1),
             save_path=None, coordinate_name=None, legend_loc='upper right'):
        ...
        ax.legend(tuple(line_lists), labels, loc=legend_loc, fontsize=13, ncol=1)

        if coordinate_name is not None:
            plt.xlabel(coordinate_name[0], fontsize=15)
            plt.ylabel(coordinate_name[1], fontsize=15)

        plt.tick_params(labelsize=15)
        plt.xticks(range(0, 6, 1), ['50', '60', '70', '80', '90', '100'])
        plt.subplots_adjust(left=0.5, right=0.985, bottom=0.113, top=0.95)
        plt.grid(linestyle=':')
